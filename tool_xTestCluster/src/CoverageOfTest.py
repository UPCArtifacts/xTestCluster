...
def runComputeCoverageForBugIdAllTestGeneration(bugid,
	...
	for tg in testGeneration:
		...
		r =  runComputeCoverageForBugId(bugid = bugid,
...
def runComputeCoverageForBugId(bugid,
							 destinationTestGenerated,
							 pathsToPatches=[],
							 outResults = os.path.realpath("./results/"),
							 isEvosuite = True,
...
def runComputeCoverageForPatch(patchPath,
	...
	if statusApplyingPatch != 0 or verificationSuccessfulyApplied != 0:
		...
		logging.debug("Step: Executed Original test on patched project".format())
